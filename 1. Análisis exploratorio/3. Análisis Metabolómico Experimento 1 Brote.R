...
data=read_excel("SHOOT target results_formato.xlsx")
...
barplot(total, las=2, col="skyblue", cex.axis=0.9, cex.names=1, cex.lab=1.4,
        font.lab=2, ylab="Cuantificación total", ylim=c(0, 2500))
...
group <- factor(c(rep("WTD0", 4), rep("WTDD6", 4), rep("WTWD6", 6),
                  rep("KOD0", 5), rep("KODD6", 6), rep("KOWD6", 5),
...
outlayers=c("OxDD6R1", "WTDD6R3")
log_data_filtered=log_data[, !(colnames(log_data) %in% outlayers)]
group2=factor(c(rep("WTD0", 4), rep("WTDD6", 3), rep("WTWD6", 6),
                rep("KOD0", 5), rep("KODD6", 6), rep("KOWD6", 5),
                rep("OxD0", 5), rep("OxDD6", 5), rep("OxWD6", 6)))