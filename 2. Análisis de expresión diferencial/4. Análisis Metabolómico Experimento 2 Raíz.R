...
# Gráfico de barras
results4$DE="NO"
results4$DE[results4$logFC > 1 & results4$P.Val < 0.05]="up"
results4$DE[results4$logFC < -1 & results4$P.Val < 0.05]="down"
results4$metabolite=rownames(results4)
results4=results4[order(results4$logFC), ]

ggplot(results4, aes(x=reorder(metabolite, logFC), y=logFC, fill=DE))+
  geom_bar(stat="identity")+coord_flip()+theme_minimal()+
  labs(x = "Metabolitos", y = "Log2 FC")+
  scale_fill_manual(values=c("up"="firebrick", "down"="steelblue", "NO"="grey70"))+
  theme(axis.title.x=element_text(size=15, face="bold", margin=margin(t=20)),
